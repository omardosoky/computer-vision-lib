...
//same code as filtering because we need high and low pass filters
...
//The hyprid image uses the frequency domain filters and passes the first one to low pass filter and the
//the second one to high pass filter
cv::Mat MakeHyprid(cv::Mat img_low,cv::Mat img_high){
    Mat  Magnitude_image, high_mask, low_mask, high, low;
    freq_filtering F1;
    low = F1.freq_filter(img_low, Magnitude_image, low_mask, "lowpass", 40);
    high = F1.freq_filter(img_high, Magnitude_image, high_mask, "highpass", 40);
    Mat hyprid(high.rows,high.cols,CV_32F);
    for(int i =0;i<high.rows;i++){
        for(int j=0;j<high.cols;j++){
            //here we just multiply by 0.5 just to make sure that the hyprid image is not too bright
            hyprid.at<float>(i,j) = 0.5*low.at<float>(i,j)+0.5*high.at<float>(i,j);
        }
    }
    return hyprid;
}
...
int main() {
	Mat img,img2, Magnitude_image, high_mask, low_mask, high, low;
    img = imread("C:\\Users\\moamen\\Desktop\\Task1 CV\\en.png", 0);
    img2= imread("C:\\Users\\moamen\\Desktop\\Task1 CV\\mor.png", 0);
    freq_filtering F1;
    low = F1.freq_filter(img, Magnitude_image, low_mask, "lowpass", 40);
    imshow("lowpass", low);

    high = F1.freq_filter(img2, Magnitude_image, high_mask, "highpass", 40);

    imshow("highpass", high);
    Mat hyprid(high.rows,high.cols,CV_32F);
    hyprid = MakeHyprid(low,high);
    cv::imshow("hyprid",hyprid);
	waitKey(0);
	return 0;
}